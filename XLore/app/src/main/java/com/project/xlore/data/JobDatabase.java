...
{
    private static final String DB_NAME = "job_database.db";
    private static volatile JobDatabase INSTANCE;

    /* Retrieves the JobDao so that it's queries can be accessed. */
    ...
    private static JobDatabase create(final Context context)
    {
        copyDatabase(context);
        return Room.databaseBuilder(context, JobDatabase.class, DB_NAME).addMigrations(MIGRATION_1_2).build();
    }
     ...
     * This is done by taking the job_database.db file found within the assets folder
